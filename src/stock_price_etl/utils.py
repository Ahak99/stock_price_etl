...
def get_company_profile(ticker, p_run_time):
    ...
    save_to_json(data, ticker, f"data/raw_data/{ticker}/{ticker}_Company_Profile", "company_profile", p_run_time.strftime("%Y-%m-%d_%H-%M"))
    return data
...
def save_csv_files(ticker, json_data):
    try:
        ...
        append_to_csv(extract_company_profile(json_data), f"data/transformed_data/{ticker}/{ticker}_Company_Profile.csv")
        append_to_csv(extract_market_data(json_data), f"data/transformed_data/{ticker}/{ticker}_Market_Data.csv")
        append_to_csv(extract_valuation_ratios(json_data), f"data/transformed_data/{ticker}/{ticker}_Valuation_Ratios.csv")
        append_to_csv(extract_dividends_returns(json_data), f"data/transformed_data/{ticker}/{ticker}_Dividends_and_Returns.csv")
        append_to_csv(extract_financial_health(json_data), f"data/transformed_data/{ticker}/{ticker}_Financial_Health.csv")
        append_to_csv(extract_growth_metrics(json_data), f"data/transformed_data/{ticker}/{ticker}_Growth_Metrics.csv")
        append_to_csv(extract_risk_assessment(json_data), f"data/transformed_data/{ticker}/{ticker}_Risk_Assessment.csv")
        append_to_csv(extract_current_prices(json_data), f"data/transformed_data/{ticker}/{ticker}_Current_Prices.csv")
        append_to_csv(extract_analyst_price_targets(json_data), f"data/transformed_data/{ticker}/{ticker}_Analyst_Price_Targets.csv")
